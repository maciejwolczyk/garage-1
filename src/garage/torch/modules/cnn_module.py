...
class CNNModule(nn.Module):
    ...
    Args:
        input_var (int): Dimension of the network input.
    ...
    def __init__(
        ...
        if paddings != 0:
            # check padding if value is not default=0
            self._check_valid_padding(paddings, kernel_sizes)
        else:
            padding = 0
    ...
    @classmethod
    def _check_valid_padding(cls, paddings, filter_sizes):
        """Check if padding shape of convolutional layers are valid for `same`\
        padding (zero padding) implementation.

        Args:
            paddings (tuple[int]): paddings
            filter_sizes (tuple[int]): filter sizes

        Raises:
            ValueError: if the padding size is not equal to the expected shape

        """
        for index, (padding,
                    filter_size) in enumerate(zip(paddings, filter_sizes)):
            if padding != 0:  # only check when padding value is not 0
                expected_padding = int((filter_size - 1) / 2)  # P = (F - 1) /2
                assert (padding == expected_padding
                        ), 'Invalid padding shape at index {}.\
                Expected to be {}, but got {} instead'.format(
                            index, expected_padding, padding)
